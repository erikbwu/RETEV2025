...
from model.eegnet import EEGNet      # Adjust the import if needed
...
class EEGKeyboard:
    ...
    def preprocess(self, window):
        ...
        """
        window = window.T  # (C, T)
        tensor = torch.from_numpy(window).unsqueeze(0).unsqueeze(0)  # (1, 1, C, T)
...
def load_model(model_path, device='cpu'):
    ...
    """
    model = EEGNet(nb_classes=2, Chans=8, Samples=250)  # Adjust Samples if buffer window differs
...
def main():
    model_path = "eegnet_best.pth"  # Adjust path if needed
